...
def getCost(instrument_id):
    time.sleep(10)
    try:
        holding = optionAPI.get_specific_position(underlying='BTC-USD', instrument_id=instrument_id)
        print(holding)
        ...
        print("getCost！", traceback.format_exc())
        time.sleep(10)
        return getCost(instrument_id)
def getbuyTop(instrument_id):
    time.sleep(10)
    try:
        result = optionAPI.get_option_instruments_summary(underlying='BTC-USD', instrument_id=instrument_id)
        buyTop = float(result['best_bid'])
        print("最大买价:", buyTop)
        return buyTop
    except:
        print("getbuyTop！", traceback.format_exc())
        time.sleep(10)
...
def getNowPrice():
    ...
    try:
        ...
        if now_price<27000 or now_price>29500:
            time.sleep(60)
            return getNowPrice()
        return now_price
    except:
        ...
        time.sleep(10)
        return getNowPrice()
def takeOrder(instrument_id,side):#'BTC-USD-210108-27500-P','sell'
    time.sleep(0.5)
    try:
        ...
        print("takeOrder！", traceback.format_exc())
        time.sleep(3)
        return takeOrder(instrument_id,side)

InstrumentId_P='BTC-USD-210108-28000-P'
InstrumentId_C='BTC-USD-210108-28500-C'
avg_cost_P=0.047
avg_cost_C=0.06
...
while True:
    ...
    # if buyTop_P / avg_cost_P > 1.07:
    #     #下单卖
    #     takeOrder(InstrumentId_P,'sell')
    #     #再下单买
    #     now_price=getNowPrice()
    #     InstrumentId_P = 'BTC-USD-210108-' + str(now_price // 500 * 500) + '-P'
    ...
    if buyTop_C / avg_cost_C > 1.07:
        # 下单卖
        takeOrder(InstrumentId_C, 'sell')
        # 再下单买
        now_price = getNowPrice()
        InstrumentId_C = 'BTC-USD-210108-' + str((now_price // 500+1) * 500) + '-C'
        print("------------------------------------------"+"此时已改标的C："+InstrumentId_C+"--------------------------------------------")
    ...
    # takeOrder = optionAPI.take_order(instrument_id='BTC-USD-210108-34500-C', side='sell', price='', size='1', client_oid='', order_type='', match_price='1')
    # print(takeOrder)
    # if takeOrder["error_code"]=="0":
    #     holdList.remove('BTC-USD-210108-34500-C')
...
# result = optionAPI.get_instruments_summary(underlying='BTC-USD', delivery='201229')
# print(result)
#
# zhangMarketList={}
# for i in result:
...
# print(buyDieKind)
# 下单
# takeOrder = optionAPI.take_order(instrument_id='BTC-USD-210108-34500-C', side='buy', price='', size='1', client_oid='', order_type='', match_price='1')
# print(takeOrder)
