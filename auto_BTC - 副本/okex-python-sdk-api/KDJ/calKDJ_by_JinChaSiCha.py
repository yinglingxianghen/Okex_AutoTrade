...
while True:
      try:
            kXian = spotAPI.get_kline(instrument_id='BTC-USDT', start='', end='', granularity='180')
            # print(len(kXian))
            print(kXian[0])
            if int(kXian[0][0][-10:-8])-lastFen==6 or int(kXian[0][0][-10:-8])-lastFen==-54:
                  ...
                  D = 2 / 3 * preD + 1 / 3 * K
                  J = 3 * K - 2 * D
                  print(str(lastFen+3)+"","K——",round(K,6),"D——",round(D,6),"J——",round(J,6))
                  if haveDuo==True and K <preK and D < preD and D >K :
                        haveDuo=False
                  if K>preK and D>K and D-K<11:
                        if status==0:
                              print("平掉所有空")
                        ...
                        status=1
                        haveDuo=True
                  elif K<preK and K>D and K-D<11:
                        if status==1:
                              print("平掉所有多")
                        result_xian_BTC = spotAPI.get_specific_ticker('BTC-USDT')
                        BTC_now_price = result_xian_BTC["last"]
