...
def take_order(side,price):
    ...
    if side=='buy':
        if price>=top:
            ...
            takeOrder = levelAPI.take_order(instrument_id='BTC-USDT', side=side, margin_trading='2',
                                    client_oid=name + str(orderNum), price=str(price), size='0.01')
    ...
    time.sleep(3)
while True:
    sleepTime=3 if rise==True else 10
    try:
        ...
        if len(pendOrders)==0:
            ...
            if now_price-minPrice  >= interval * 3.5  or now_price - maxPrice >= interval*1.5:
                ...
                revokeOrder = levelAPI.revoke_order(instrument_id='BTC-USDT', client_oid=Oid)
